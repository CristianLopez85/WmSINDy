# -*- coding: utf-8 -*-
"""
Created on Fri Jan 17 14:27:04 2025
...
from utils_NSS_SINDy_W_LV import *
import time
...
from utils_WSINDy_LV import *

""" 
If using grid points, please change the following:
In the file 'utils_NSS_SINDy_W':
   1.- Check that LibGPU(x) is for VdP/Duffing Oscillator/ Cubic Osc/LotkaVolt
        # Theta=np.ones((n,1),dtype=float)
          Theta=x[:,[0]]
   2.- Line 96, modify as: for i in range(1,m)
In the file 'utils_WSINDy':
   3.- In def get_tags_SINDy(n): Use if n ==3:   # For VanderPol, and Cubic, Duffing
...
x = odeint(Lotka,x0,tobs,args=(p0,),rtol = 1e-12, atol = 1e-12)
dx = np.transpose(Lotka(np.transpose(x), 0, p0))             # derivative of the trajectory
...
pp1=plt.plot(xn[:,0],xn[:,1],linewidth=1.0,color='blue',linestyle='--')
plt.ylabel('x')
plt.grid(False)
plt.axis('off')

# plt.xlabel('z');plt.ylabel('y');plt.xlabel('x');plt.grid(True)
plt.savefig('Fig7f_LotkaVol_noisy.pdf')
rr
...
plt.figure()
pp1=plt.plot(x[:,0],x[:,1],linewidth=lw,color='k',linestyle='-')
pp1=plt.plot(xn[:,0]-NoiseID[:,0],xn[:,1]-NoiseID[:,1],linewidth=lw,color='cyan',linestyle='--')
plt.ylabel('x')
plt.grid(False)
plt.axis('off')
plt.savefig('Fig7f_LotkaVolt_denoised.pdf')
...
Coll_Errs = [Enoise_error,Evector_field_error,Epre_error, ParameterError]

# print("Enoise_error,Evector_field_error,Epre_error,  ParameterError:",Coll_Errs)   
