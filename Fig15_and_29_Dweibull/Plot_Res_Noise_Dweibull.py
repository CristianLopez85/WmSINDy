...
Res_mSINDy = scipy.io.loadmat('mLorenz_Dweibull.mat') 
Res_WmSINDy = scipy.io.loadmat('WmLorenz_Dweibull.mat')  
Res_WSINDy = scipy.io.loadmat('WLorenz_Dweibull.mat')
...
ax1.yaxis.grid(True)
plt.ylim([1e-4, 1])   # plt.ylim([1e-4, 1e0])
plt.savefig("p28d.pdf")
...
Wm_noise_ids_at_50_percent_last_k = Wm_NoiseIDList[:, 9, 5, :, :]
...
# 10 for 50% noise,                                    5 for last Nloop
m_noise_ids_at_50_percent_last_k = m_NoiseIDList[:, 9, 5, :, :]
...
pin = 0
NoisePercentage = 45  # -----> match with: j = NoiseNum - 1
