...
N_of_Loops=[1,2,3,4,5,6,7,8]                        # Added by CL
#%% Plot the parameter error
NoiseNum = len(N_of_Loops)

Res_mSINDy = scipy.io.loadmat('m_CF.mat')  # Load the MATLAB .mat file into Python
Res_WmSINDy = scipy.io.loadmat('Wm_CF.mat')  # Load the MATLAB .mat file into Python

Nloop = 5

WmNoise_Error = Res_WmSINDy['E_noise'].squeeze()
mNoise_Error = Res_mSINDy['E_noise'].squeeze()
...
ax1.set_xticklabels(N_of_Loops)
...
plt.yscale('log')
plt.yticks([1e1,1e0,1e-1])
plt.ylim([0.07, 80])   # plt.ylim([1e-4, 1e0])
ax1.yaxis.grid(True)

plt.savefig("p2a.pdf")

#%%
WmField_Error = Res_WmSINDy['E_vector_field'].squeeze()
mField_Error = Res_mSINDy['E_vector_field'].squeeze()
...
#%%
Wm_short_traj_Error = Res_WmSINDy['E_short_traj'].squeeze() 
m_short_traj_Error = Res_mSINDy['E_short_traj'].squeeze()

quartile2a, median2, quartile2b = np.percentile(Wm_short_traj_Error, [25, 50, 75], axis=0)
quartile4a, median4, quartile4b = np.percentile(m_short_traj_Error, [25, 50, 75], axis=0)
...
# Plotting on the first axis
line1, = ax1.plot(median2,linewidth=lw1,color=color_WmS,alpha=0.6,zorder=0,linestyle='solid')
sns.violinplot(data=Wm_short_traj_Error,cut=0,inner="box",width=comp_f*width_violin,scale="width",color=color_WmS,linewidth=comp_f*lw2,saturation=transAlpha,zorder=0, ax=ax1)
ax1.scatter(range(NoiseNum),median2,s=MarkerSize,marker='o',color=color_WmS,edgecolors=edge_color,linewidth=lw3)
ax1.set_xticks(range(NoiseNum))
ax1.set_xticklabels(N_of_Loops)
plt.yscale('log')
...
# Plotting on the second axis
line2, =  ax2.plot(median4,linewidth=lw1,color=color_NSS_SINDy,alpha=0.6,zorder=0,linestyle='solid')
sns.violinplot(data=m_short_traj_Error,cut=0,inner="box",width=comp_f*width_violin,scale="width",color=color_NSS_SINDy,linewidth=comp_f*lw2,saturation=transAlpha,zorder=0, ax=ax2)
ax2.scatter(range(NoiseNum),median4,s=MarkerSize,marker='o',color=color_NSS_SINDy,edgecolors=edge_color,linewidth=lw3)
ax2.set_xticks(range(NoiseNum))
...
plt.yscale('log')
plt.yticks([1e0,1e-1,1e-2,1e-3])
ax1.yaxis.grid(True)
# plt.ylim([2*1e-4, 2])   # plt.ylim([1e-4, 1e0])
plt.savefig("p2d.pdf")

#%%
WmParameter_Error = Res_WmSINDy['E_parameter'].squeeze() 
mParameter_Error = Res_mSINDy['E_parameter'].squeeze()
...
#%%
WmR_o_S_Error = Res_WmSINDy['Rate_success'].squeeze()   ; WmR_o_S_Error = 2.0*np.sum(WmR_o_S_Error,axis=0)  
mR_o_S_Error = Res_mSINDy['Rate_success'].squeeze()  ; mR_o_S_Error = 2.0*np.sum(mR_o_S_Error,axis=0) 
...
plt.xticks(range(NoiseNum),N_of_Loops)
