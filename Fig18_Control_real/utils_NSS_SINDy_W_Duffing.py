...
# =============================================================================
# Define the library function for the SINDy as in EDDI
# =============================================================================
def Lib(x,libOrder):
    ...
    Theta=np.concatenate([np.ones((n,1),dtype=float),x1,x2,x1**2,x1*x2,x2**2],axis=1)
...
def LibGPU(x):
    ...
    # Theta=tf.concat([tf.ones_like(z1),z1,z2,z1**2,z1*z2,z2**2,z1**3,(z1**2)*z2,z1*(z2**2),z2**3],axis=1)  
    
    # Manual \Theta
    Theta = tf.concat([tf.ones_like(z1),z1,z2,z1**2,z1*z2,z2**2],axis=1)
    
    # =============================================================================
    # Following is the 5th order lib for 2 dimentional system   
    # =============================================================================
    # For VanderPol, Duffing, and Cubic Osc
    
    # Theta=tf.concat([z1,z2,z1**2,z1*z2,z2**2,z1**3,(z1**2)*z2,z1*(z2**2),z2**3,
    #                  z1**4, (z1**3)*z2, (z1**2)*z2**2, z1*z2**3, z2**4, z1**5,
    #                 (z1**4)*z2, (z1**3)*z2**2, (z1**2)*z2**3, z1*z2**4, z2**5],axis=1) 
...
def RK45_F_SINDy(xin,LibGPU,Xi,dt):
    ...
    return tf.math.add_n([tf.reshape(xin[:, 0], (len(xin), 1)),tf.constant(1/6)*K1,tf.constant(1/3)*K2,tf.constant(1/3)*K3,tf.constant(1/6)*K4])
...
# =============================================================================
@tf.function                                                                  
def OneStepLoss_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,LibGPU,optimizer,Tags,V, Vp, Grid,U): 
    with tf.GradientTape() as g:
        ...
        v_trimmed = U[q:len(U)-1]                               #-----------------
        v_tensor = tf.cast(v_trimmed, dtype=tf.float32)
        v_tensor = tf.reshape(v_tensor, (len(v_trimmed), 1))
        X0 = tf.concat([X0, v_tensor], axis=1)  
        ...
        U_tensor = tf.reshape(U, (len(U), 1))
        Xes = tf.concat([Xes, tf.cast(U_tensor, dtype=tf.float32)], axis=1)  
        ### Added by CL
...
def Weight_Residual(xin,Xi_var,stateVar,Tags,V,Vp,Grid):
    ...
    b_Gw = []
    for nn in range(xin.shape[1]-1):                  # Added -1 for Real
        y = []; v_n = []; vp_n = []; b = []; 
...
# =============================================================================
def Train_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,optimizer,update_each,Tags, V, Vp, Grid,v_tf): 
    
    start = time.time() 
    for i in range(update_each):   # Before: N_train
        # Calculate the cost and updte the gradient        
        J = OneStepLoss_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,LibGPU,optimizer,Tags,V, Vp, Grid,v_tf) 
