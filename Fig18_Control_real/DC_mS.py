...
hop = 1
t   = data['exp_data'][:,0][::hop]
theta  = data['exp_data'][:,2][::hop]
v      = data['exp_data'][:,1][::hop]
...
dxes=CalDerivative(xes,dt,1)   # Dimension added
...
x_both = np.column_stack((theta, v))                           #-------------
...
v_trimmed = v[q:len(v)-1]                               #-----------------

for k in range(Nloop):
    print("Runing the loop ",str(k+1))
    # Denoise the signal
    NoiseID,totalTime=Train_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,optimizer,N_train,v_trimmed)
    ...
    x_both = np.column_stack((xes, v[q+1:-q-1]))                           #-------------
