...
def findcorners(xn,t,tau,tauhat,phi_class):
    ...
    for nn in range(n):
        corner = findcornerpts(xn, t)             # ---- For real, xn[:, nn] to xn
        k = corner[1]
...
def get_tags_SINDy(n):
    ...
    if n ==1:
        tags = np.array([[0, 0] , [1, 0] ,[0, 1] , [2, 0] , [1, 1] ,[0 ,2] ])

    # if n ==3:  # For Rossler
    #     tags = np.array([[0, 0] , [1, 0] ,[0, 1] , [2, 0] , [1, 1] ,[0 ,2],[3,0],[2,1],[1,2],[0,3] ])
...
def wsindy_ode_fun(xobs,tobs,
    ...
    if np.any(tau > 1):
        ...
        if tauhat > 0:
            ...
            mts, pts, ks = findcorners(xobs[:,1], tobs, tau, -tauhat, phi_class)    # For Motor real data [:,1]
        else:
