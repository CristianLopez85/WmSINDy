...
# Data smoothing
smoothing_window = 0 #int(np.ceil(len(t) / 100))   # rough guess for smoothing window. DEFAULT: smoothing_window = ceil(length(tobs)/100).
...
Xi_base[2,1] =  3;  Xi_base[2,2] = -8/3
Xi_base[4,0] =  1;  Xi_base[4,2] = 1
Xi_base[5,1] = -1
...
for i in range(N_run):
    ...
    for j in range(NoiseNum):
        ...
        # Xi0[0,0]=-9.5;Xi0[1,0]=10.5;                                         # ! Commented and lines 378-380
        ...
        for k in range(Nloop):
            ...
            # Add constraint                                                    
            # index_min[0,0]=True;index_min[1,0]=True;                         # ! Commented and lines 326-328
...
#%%
data_dict = {"E_noise": Enoise_error_List, "E_vector_field": Evector_field_error_list, 
             "E_traj": Epre_error_list,"E_short_traj": Epre_short_error_list,"E_parameter": Epar_error_list,
             "Rata_succes": SuccessOrNot_list}
savemat("Wm_no_DisM.mat", data_dict)
