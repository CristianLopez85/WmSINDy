...
Res_mSINDy = scipy.io.loadmat('mLorenz_gamma.mat') 
Res_WmSINDy = scipy.io.loadmat('WmLorenz_gamma.mat')  
Res_WSINDy = scipy.io.loadmat('WLorenz_gamma.mat')
...
ax1.yaxis.grid(True)
plt.ylim([0.5*1e-6, 40])   # plt.ylim([1e-4, 1e0])
plt.savefig("p27b.pdf")
...
Wm_noise_ids_at_50_percent_last_k = Wm_NoiseIDList[:, 2, 5, :, :]
...
# 10 for 50% noise,                                    5 for last Nloop
m_noise_ids_at_50_percent_last_k = m_NoiseIDList[:, 2, 5, :, :]
...
pin = 0
NoisePercentage = 10   # -----> match with: j = NoiseNum - 1
