...
gamma_params = best_fit['gamma']
k = gamma_params['a']  # shape parameter
loc = gamma_params['loc']  # location parameter
theta = gamma_params['scale']  # scale parameter

# Calculate theoretical moments from the fitted parameters
fitted_mean = loc + k*theta
fitted_std = np.sqrt(k)*theta

# Calculate actual moments directly from the data
actual_mean = np.mean(Noise[:,0])
actual_std = np.std(Noise[:,0])

print(f"Fitted parameters: shape(k)={k}, loc={loc}, scale(theta)={theta}")
