...
for i in range(N_run):
    ...
    for j in range(NoiseNum):
        ...
        # Generate the noise
        NoiseMag = [np.std(x[:,i])*NoisePercentageToSwipe[j]*0.01 for i in range(stateVar)] # Before in line 275
        
        for k in range(stateVar):
            
            desired_std = NoiseMag[k]
...
savemat("WmLorenz_Rayleigh.mat", data_dict)