...
Res_mSINDy = scipy.io.loadmat('mLorenz_Rayleigh.mat') 
Res_WmSINDy = scipy.io.loadmat('WmLorenz_Rayleigh.mat')  
Res_WSINDy = scipy.io.loadmat('WLorenz_Rayleigh.mat')
...
ax1.yaxis.grid(True)
plt.ylim([0.5*1e-6, 4])   # plt.ylim([1e-4, 1e0])
...
Wm_noise_ids_at_50_percent_last_k = Wm_NoiseIDList[:, 5, 5, :, :]
...
# 10 for 50% noise,                                    5 for last Nloop
m_noise_ids_at_50_percent_last_k = m_NoiseIDList[:, 5, 5, :, :]
...
pin = 0
NoisePercentage = 25   # -----> match with: j = NoiseNum - 1
