# -*- coding: utf-8 -*-
"""
Created on Tue Jun 20 16:00:23 2023
...
def sphs(Pf, K, t):
    ...
    u = np.sqrt(2 / K) * np.cos((2 * np.pi * i[:, np.newaxis] * t / Pf + theta[:, np.newaxis])/1)    # reduce freq: 10
...
# Run the simulation
tobs, x, u, dt, dx = getTrainingData()
...
# Get the initial guess of Xi from WSINDy
                                                                           # u, added for control 
w_sparse, v, vp, mts,pts, grid_or, loss_wsindy, tags = wsindy_ode_fun(xobs,u,tobs,
...
def v_vp_grid(xn_in ,NoiseID_in, K_frac, tags, stateVar,custom_fcns, tobs, tau, tauhat, max_d, phi_class):
...
for k in range(Nloop):
    ...
    for kk in range(N_train_mod):
        ...
        NoiseID,totalTime = Train_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,optimizer,update_each, V,Vp, Grid, U, U0) 
        
        v_up, vp_up, grid_up = v_vp_grid(xn ,NoiseID, K_frac, tags, stateVar,custom_fcns, tobs, tau, tauhat, max_d, phi_class)
