...
# =============================================================================
@tf.function
def OneStepLoss_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,LibGPU,optimizer,V, Vp, Grid, U, U0):
    with tf.GradientTape() as g:
        ...
        ### Added by CL
        Jr = 1.0*Weight_Residual(Xes,Xi_var,stateVar,V,Vp,Grid, U)
...
# =============================================================================
# Define a function that calculate the residual mean square error
# =============================================================================
@tf.function
def Weight_Residual(xin,Xi_var,stateVar,V,Vp,Grid, u):
        
    wTheta = tf.transpose(LibGPU(xin, u))           
    ...
    for nn in range(xin.shape[1]):
        ...
        Theta_4d = tf.reshape(wTheta, [1] + wTheta.shape.as_list() + [1])
        ...
        Gw = tf.linalg.matmul(G, tf.reshape(Xi_var[:,nn], (LibGPU(xin, u).shape[1], 1)))
...
# =============================================================================
def Train_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,optimizer,N_train, V, Vp, Grid, U, U0):
    start=time.time() 
    for i in range(N_train):
        # Calculate the cost and updte the gradient
        J=OneStepLoss_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,dataLen,LibGPU,optimizer,V, Vp, Grid, U, U0)
