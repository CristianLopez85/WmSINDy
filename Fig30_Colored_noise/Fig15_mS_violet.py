# -*- coding: utf-8 -*-
"""
Created on Sat Mar  8 20:04:48 2025
...
def colored_noise_generator(N):
    ...
    # S = 1/np.where(f == 0, float('inf'), f)           # Brownian noise
    S = f                                             # Violet noise
    # Normalize S
...
NoisePercentageToSwipe=[0,5,10,15,20]
...
for i in range(N_run):
    ...
    for j in range(NoiseNum):
        ...
        # Generate blue noise for each state variable
        ...
        for ii in range(stateVar):
            ...
            NoiseMag = np.std(x[:, ii]) * NoisePercentageToSwipe[j] * 0.01
        ...
        if Softstart==1:
            ...
            NoiseEs = np.zeros((xn.shape[0],xn.shape[1]))             # by CL
            NoiseVar = tf.Variable(NoiseEs,dtype=tf.dtypes.float32)
...
data_dict = {"NoiseID": NoiseIDList, 
             "Time": TrainTimeList, "E_noise": Enoise_error_List, "E_vector_field": Evector_field_error_list, 
             "E_traj": Epre_error_list,"E_short_traj": Epre_short_error_list,"E_parameter": Epar_error_list,
             "Rate_success": SuccessOrNot_list}
savemat("mLorenz_violet.mat", data_dict)