...
# Define the color
color_WmS_Drop = 'pink'   #[0.58, 0.44, 0.86]
color_NSS_SINDy_Drop = 'orange'      #'c'
...
import scipy.io
Res_mSINDy = scipy.io.loadmat('mS_full.mat')  # Load the MATLAB .mat file into Python
Res_WmSINDy = scipy.io.loadmat('Wm_full.mat')  # Load the MATLAB .mat file into Python
...
plt.savefig("p9r_drop.pdf")

#%% Incomplete Loss function
NoiseNum = 4

import scipy.io
Res_mSINDy = scipy.io.loadmat('mS_Drop_t.mat')  # Load the MATLAB .mat file into Python
Res_WmSINDy = scipy.io.loadmat('Wm_Drop_t.mat')  # Load the MATLAB .mat file into Python
...
ChangeFontSize(30)
# Plotting on the first axis
line1, = ax1.plot(median1,linewidth=lw1,color=color_WmS_Drop,alpha=0.6,zorder=0,linestyle='solid')

sns.violinplot(data=WmSINDy,cut=0,inner="box",width=width_violin,scale="width",color=color_WmS_Drop,linewidth=lw2,saturation=transAlpha,zorder=0, ax=ax1)
ax1.scatter(range(NoiseNum),median1,s=MarkerSize,marker='o',color=color_WmS_Drop,edgecolors=edge_color,linewidth=lw3)
ax1.set_xticks(range(NoiseNum))
ax1.set_xticklabels(LengthToSwipe)
plt.yscale('log')
plt.ylabel('Error', fontsize=30);
...
# Plotting on the second axis
line2, =  ax2.plot(median3,linewidth=lw1,color=color_NSS_SINDy_Drop,alpha=0.6,zorder=0,linestyle='solid')
sns.violinplot(data=mSINDy,cut=0,inner="box",width=width_violin,scale="width",color=color_NSS_SINDy_Drop,linewidth=lw2,saturation=transAlpha,zorder=0, ax=ax2)
ax2.scatter(range(NoiseNum),median3,s=MarkerSize,marker='o',color=color_NSS_SINDy_Drop,edgecolors=edge_color,linewidth=lw3)
ax2.set_xticks(range(NoiseNum))
# ax2.set_xticklabels(LengthToSwipe2)
...
# Plotting bars
plt.bar(labels, heights, color=[color_NSS_SINDy_Drop, color_WmS_Drop], width=bar_width, edgecolor='black')
plt.yticks([25,50,75,100])
plt.grid(axis='y', color='gray', linestyle='-', alpha=0.7)

plt.savefig("p9r_full.pdf")