...
def calculo(Longitud):
    ...
    _, v, vp, _,_, grid_or, _, _ = wsindy_ode_fun(x_full_length,t,
    ...
    for i in range(N_run):
        ...
        for j in range(DataLengthNum):
            ...
            Xi0, v, vp, mts,pts, grid_or, loss_wsindy, tags = wsindy_ode_fun(xes,tes,
                                                          polyorder,    custom_tags,custom_fcns,
                                                          phi_class,max_d,tau,tauhat,K_frac,overlap_frac,relax_AG,
                                                          scale_Theta,useGLS,lambdas,gamma,alpha_loss,
                                                                                                    0) # smoothing_window 
            ...
            for k in range(Nloop):
                ...
                NoiseID,totalTime = Train_NSS_SINDy(Y,Y0,Ypre_F,Ypre_B,NoiseVar,Xi,Xi_act,weights,dt,q,stateVar,Trimed_dataLen,optimizer,N_train,Tags, V,Vp, Grid) 
                ...
                # df.to_excel(f'WmS_Time{Longitud}.xlsx', index=False)
                            
                # # Save Enoise_error_list
                # df_noise = pd.DataFrame(Enoise_error_List.reshape(N_run,Nloop))
                # df_noise.to_excel(f'WmS_E_noise{Longitud}.xlsx', index=False)
        
                # # Save Evector_field_error_list
                # df_vector = pd.DataFrame(Evector_field_error_list.reshape(N_run,Nloop))
                # df_vector.to_excel(f'WmS_E_vector_field{Longitud}.xlsx', index=False)
                        
                # # Save Evector_field_error_list
                # df_traj = pd.DataFrame(Epre_error_list.reshape(N_run,Nloop))
                # df_traj.to_excel(f'WmS_E_pre_{Longitud}.xlsx', index=False)
                        
                # # Save Eshort_error_list
                # df_short = pd.DataFrame(Epre_short_error_list.reshape(N_run,Nloop))
                # df_short.to_excel(f'WmS_E_short_{Longitud}.xlsx', index=False)
                        
                # # Save Eshort_error_list
                # df_parameter = pd.DataFrame(Epar_error_list.reshape(N_run,Nloop))
                # df_parameter.to_excel(f'WmS_E_parameter_{Longitud}.xlsx', index=False)
                        
                # # Save ESuccess_list
                # df_success = pd.DataFrame(SuccessOrNot_list.reshape(N_run,Nloop))
                # df_success.to_excel(f'WmS_Success_{Longitud}.xlsx', index=False)
                
                savemat(f'WmS_noAppN_{Longitud}.mat', {
